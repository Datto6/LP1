...
Caixa* concatena (struct Caixa* l1, struct Caixa* l2) { //concatena duas listas encadeadas, sem repeticao
	Caixa* p=l1;
    Caixa* ultimo=NULL;
	while (p!=NULL){
		if (contem(l2,p->valor)){
			removeLista(&l2,p->valor);
		}
        ultimo=p;
		p=p->prox;
	}
...
int main(void){
    ...
    coisa7=constroi(&coisa8,5);
    coisa8=constroi(NULL,9);
